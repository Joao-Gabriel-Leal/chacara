...
import { eventSettings, profiles } from "@/lib/mock-data";
import { appEnv } from "@/lib/env";

export default function LandingPage() {
  const startsAt = new Date(appEnv.eventDate || eventSettings.startsAt);
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {[
                ...
                { label: "Convidados", value: `${profiles.length}` },
              ].map((item) => (
        ...
        <MotionFade delay={0.1}>
          <div className="rounded-[36px] border border-white/10 bg-black/35 p-5 shadow-[0_50px_120px_-50px_rgba(16,185,129,0.45)] backdrop-blur-2xl">
            <div className="rounded-[30px] border border-white/10 bg-gradient-to-br from-emerald-400/20 to-cyan-400/10 p-5">
              ...
              <p className="mt-3 max-w-md text-sm text-zinc-200">{eventSettings.tagline}</p>
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {[
                  { icon: CalendarRange, label: "Data", value: "12 Dez 2026" },
                  { icon: CreditCard, label: "Cota", value: "R$ 240 por pessoa" },
                  { icon: Images, label: "Galeria", value: "Album + app + curadoria" },
                  { icon: Users, label: "Grupo", value: "20 pessoas confirmadas" },
              ...
              <div className="mt-6 flex items-center gap-3">
                ...
                <p className="text-sm text-zinc-300">{eventSettings.location}</p>
              </div>
            </div>
          </div>
        </MotionFade>
